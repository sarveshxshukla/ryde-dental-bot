...
(function () {
  ...
  var css = "" +
    ...
    "#rdf-panel{position:fixed;right:18px;bottom:86px;width:380px;max-width:calc(100vw - 28px);height:620px;max-height:calc(100vh - 106px);background:#fff;border:1px solid #eef0f2;border-radius:20px;overflow:hidden;display:none;flex-direction:column;z-index:2147483000;box-shadow:0 24px 60px rgba(10,63,61,.25)}" +"#rdf-panel.fit{height:auto!important;max-height:calc(100vh - 100px)}" +"#rdf-panel.fit #rdf-body{flex:1 1 auto;overflow-y:auto;min-height:0}" +
    ...
    "#rdf-head{position:relative;z-index:6;padding:14px 12px 14px 18px;display:flex;align-items:center;gap:12px;background:#fff;border-bottom:1px solid #eef0f2}" +
    ...
    "#rdf-book{background:#123C4A;border:none;color:#fff;cursor:pointer;width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;margin-left:auto;flex-shrink:0}" +
    "#rdf-x{margin-left:8px;background:#123C4A;border:none;color:#fff;cursor:pointer;font-size:23px;width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:background .12s,color .12s}#rdf-x:hover{background:#0C2C36;color:#fff}" +
    ...
    ".rdf-wrap{max-width:82%}" +
    "@keyframes rdfin{from{opacity:0;transform:translateY(6px)}to{opacity:1}}" +
    ".rdf-b{width:fit-content;min-width:0;padding:10px 14px;font-size:14.5px;line-height:1.5;white-space:normal;overflow-wrap:break-word;word-break:normal;hyphens:none}" +
    ".rdf-b.bot{background:#f4f6f8;color:#2b3138;border:none;border-radius:4px 18px 18px 18px}" +
    ".rdf-b.me{background:" + C.teal + ";color:#fff;border-radius:18px 4px 18px 18px}" +
    ...
    ".rdf-chip{padding:8px 15px;font-size:13.5px;font-weight:500;border-radius:22px;background:#fff;color:" + C.teal + ";border:1.5px solid #F7C9AC;cursor:pointer}" +
    ".rdf-chip:hover{transform:translateY(-1px)}" +
    ".rdf-cta{display:inline-flex;align-items:center;gap:5px;padding:6px 12px;font-size:13px;font-weight:600;border-radius:20px;background:#1FA463;color:#fff !important;border:1px solid #1FA463;text-decoration:none;cursor:pointer;transition:background .12s}" +
    ".rdf-cta:hover{background:#178a52}" +
    "#rdf-foot{padding:14px 16px;display:flex;align-items:center;gap:10px;border-top:1px solid #eef0f2;background:#fff}" +
    "#rdf-inwrap{flex:1;display:flex;align-items:center;gap:4px;background:#f4f6f8;border:1px solid #eef0f2;border-radius:24px;padding:0 6px 0 16px}" +
    ...
    "#rdf-form{position:absolute;left:0;right:0;top:68px;bottom:0;background:#fff;z-index:5;display:flex;flex-direction:column;animation:rdfin .2s ease}" +
    "#rdf-intake{flex:1;min-height:0;display:flex;flex-direction:column;padding:0;width:100%;animation:rdfin .2s ease}" +
    ...
    "#rdf-body.intake{padding:0}" +
    ".rdf-iw{position:relative;display:flex;flex-direction:column;gap:18px;padding:18px 20px 20px;min-height:0}" +"@media (max-height:720px){.rdf-iw{gap:12px;padding:14px 20px 16px}.rdf-fi{padding:8px 2px}.rdf-ita{min-height:44px}.rdf-it{font-size:17px}}" +
    ...
    "#rdfw #rdf-book{width:38px!important;height:38px!important;padding:0!important;border-radius:50%!important;background:#123C4A!important;border:none!important;color:#fff!important;box-shadow:none!important;display:flex!important;align-items:center;justify-content:center;flex-shrink:0}" +"#rdfw #rdf-x{width:38px!important;height:38px!important;padding:0!important;border-radius:50%!important;background:#123C4A!important;border:none!important;color:#fff!important;box-shadow:none!important;display:flex!important;align-items:center;justify-content:center;flex-shrink:0}" +
    "#rdfw #rdf-x{font-size:23px!important;margin-left:8px!important;color:#fff!important}" +
  ...
  function typing(on) {
    ...
    var wrap = el("div", "rdf-wrap");
    wrap.appendChild(el("div", "rdf-b bot", '<span class="rdf-dot"></span> <span class="rdf-dot" style="animation-delay:.15s"></span> <span class="rdf-dot" style="animation-delay:.3s"></span>'));
    row.appendChild(wrap);
  ...
  function setMode(m) {
    ...
    $("rdf-stt").textContent = m === "human" ? "Reception team · online" : "Replies instantly";
    $("rdf-dot2").style.background = m === "human" ? "#FFE0C8" : "#FFFFFF";
    $("rdf-head").querySelector(".av").innerHTML = m === "human" ? head : toothLogo;
  }
  ...
  function showIntake() {
    ...
    var pn = $("rdf-panel"); if (pn) pn.className = (pn.className.replace(/\bfit\b/,"").trim() + " fit").trim();
  ...
  function submitIntake() {
    ...
    fetch(API + "/api/start", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: SID, name: name, phone: phone, email: email, message: msg }) })
      ...
      .then(function () {
        ...
        var pn2 = $("rdf-panel"); if (pn2) pn2.className = pn2.className.replace(/\bfit\b/,"").trim();
        body.className = "";
        body.className = "";
