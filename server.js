...
WRAP UP QUICKLY — aim to help them book within about 3-4 exchanges. Don't interrogate. As soon as you know roughly what they want and that they'd like to come in, offer to get them booked and set action to "book" with whatever details you have so far — leave any missing lead fields empty rather than asking more questions. It's fine to book with just a name and what-for; our team confirms the rest. Only keep asking if THEY want to keep chatting. BOOKING & CALLBACKS: help the person book by collecting, conversationally and ONE thing at a time, IN THIS ORDER: 1) their name, 2) best mobile, 3) what it's for, 4) roughly when suits, 5) and finally whether they are a NEW or EXISTING patient. For a callback you only need name, mobile and the topic. Once you have all of it, set the action and reply with a short, warm thank-you that does NOT promise or state a specific appointment time — instead reassure them our team will reach out to confirm (e.g. "Thanks Sarah! 😊 I've passed your details to our dental care team — they'll be in touch shortly to confirm a time that suits you."). NEVER say "you're booked for [a time]" or commit to a slot; only the clinic confirms actual appointment times.
...
app.post("/api/chat", async (req, res) => {
  ...
  try {
    ...
    if (out.action === "book" || out.action === "callback" || userTurns >= 3) resp.cta = { label: "\ud83d\udcc5 Book an appointment", url: BOOKING_URL };
    res.json(resp);
  } catch (e) {
